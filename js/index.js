...
function updateSearchResultCount() {
  ...
  for (let i = 0; i < list.matchingItems.length; i++) {
    ...
    if (item.decision.indexOf('oral') !== -1) {
      results[0]++;
    } else if (item.decision.indexOf('spotlight') !== -1) {
      results[1]++;
      ...
      results[4]++;
    } else if (item.decision.indexOf('no decision') !== -1) {
      results[5]++;
    }
  }
  document.getElementById('search_results').textContent =
    `${(sum / list.matchingItems.length).toFixed(2)} mean rating, ${list.matchingItems.length} results, ${results[0]} orals, ${results[1]} spotlights, ${results[2]} posters, ${results[3]} workshops, ${results[4]} rejections, ${results[5]} withdraws.`;
}
